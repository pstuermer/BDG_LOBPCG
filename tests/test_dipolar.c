...
TEST(kernel_r2c_vs_c2c) {
    ...
    const uint64_t N0k_r2c = Nx / 2 + 1;  /* 5 */
    const uint64_t N0k_c2c = Nx;           /* 8 */

    for (uint64_t iz = 0; iz < Nz; iz++) {
        for (uint64_t iy = 0; iy < Ny; iy++) {
            for (uint64_t ix = 0; ix <= Nx / 2; ix++) {
                const uint64_t idx_r2c = iz * Ny * N0k_r2c + iy * N0k_r2c + ix;
                const uint64_t idx_c2c = iz * Ny * N0k_c2c + iy * N0k_c2c + ix;
    ...
    for (uint64_t iz = 0; iz < N[2]; iz++)
        for (uint64_t iy = 0; iy < N[1]; iy++)
            for (uint64_t ix = 0; ix < N[0]; ix++) {
                ...
                const uint64_t idx = iz * N[1] * N[0] + iy * N[0] + ix;
