...
matmul_ctx_t *matmul_ctx_alloc(uint64_t dim, const uint64_t *N, const f64 *L) {
  ...
  ctx->N = xcalloc(dim, sizeof(uint64_t));
...
void matmul_ctx_set_system(matmul_ctx_t *ctx, int complex_psi0) {
  ...
  const uint64_t N0_k = complex_psi0 ? ctx->N[0] : (ctx->N[0] / 2 + 1);
  ctx->k2 = xcalloc(ctx->k_size, sizeof(f64));

  if (1 == ctx->dim) {
    for (uint64_t ix = 0; ix < N0_k; ix++)
      ctx->k2[ix] = ctx->kx2[0][ix];
  } else if (2 == ctx->dim) {
    for (uint64_t iy = 0; iy < ctx->N[1]; iy++)
      for (uint64_t ix = 0; ix < N0_k; ix++)
	ctx->k2[iy * N0_k + ix] = ctx->kx2[0][ix] + ctx->kx2[1][iy];
  } else {
    for (uint64_t iz = 0; iz < ctx->N[2]; iz++)
      for (uint64_t iy = 0; iy < ctx->N[1]; iy++)
	for (uint64_t ix = 0; ix < N0_k; ix++)
  ...
  const uint64_t wrk_size = (ctx->size > ctx->k_size) ? ctx->size : ctx->k_size;
